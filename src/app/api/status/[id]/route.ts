...
export async function POST(req:Request) {
  try {
    ...
    if (user && transactionStatus == "SUCCESS") {
      user.purchases.push({
        roadmapId: roadmapid,
        amount:20,
      ...
      await user.save();
      alert("Purchase successful. Access roadmaps through services.")
    } else {
      console.log("Couldn't find")
    }
    // Redirect based on transaction status
    const redirectUrl = transactionStatus === "SUCCESS" ? `${process.env.NEXTAUTH_URL}` : `${process.env.NEXTAUTH_URL}/failure`;
